  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Link to="/brands/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-blue-700">
          Create Campaign
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-gray-500 text-sm font-medium">Total Budget Allocated</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="py-3 px-4 font-semibold text-gray-700">Budget</th>
              ...
              <tbody>
                {campaigns.map(campaign => (
                  <tr key={campaign.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4 font-medium text-gray-900">{campaign.title}</td>
                    ...
                    <td className="py-3 px-4">
                      <Link to={`/brands/campaign/${campaign.id}`} className="text-blue-600 hover:text-blue-800 font-medium text-sm">
                        View Submissions
                      </Link>
                    </td>
                  </tr>
...
function CampaignDetails() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-900">{campaign.title} - Submissions</h2>
        <Link to="/brands" className="text-gray-500 hover:text-gray-700">Back to Dashboard</Link>
      </div>

      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        {submissions.length === 0 ? (
          <div className="text-center py-12 text-gray-500">No content submitted yet.</div>
        ) : (
          <div className="space-y-4">
            {submissions.map(sub => (
              <div key={sub.id} className="border border-gray-200 rounded-lg p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <a href={sub.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-medium break-all">
                    {sub.url}
                  </a>
                  <p className="text-sm text-gray-500 mt-1">Submitted: {new Date(sub.submittedAt).toLocaleDateString()}</p>
                  <p className="text-sm text-gray-500">FTC Compliant: {sub.ftcAgreed ? 'Yes' : 'No'}</p>
                </div>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-gray-100">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/brands" className="text-gray-500 hover:text-gray-700">Cancel</Link>
      </div>
